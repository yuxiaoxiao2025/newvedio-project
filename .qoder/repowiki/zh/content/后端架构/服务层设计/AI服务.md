...
- 新增文件路径转换和环境变量加载机制
- **改进JSON解析失败时的时长提取逻辑，从原始文本中提取duration信息**
...
- **增强的错误恢复能力**：在JSON解析失败时，从原始文本中提取视频时长信息
...
当`analyzeVideoContent`方法调用VL模型进行视频分析时，如果返回的JSON格式解析失败，系统会尝试从原始文本中提取视频时长信息，以避免返回硬编码的0值。

**节来源**
- [aiService.js](file://backend/src/services/aiService.js#L170-L184)

#### 时长提取流程

```mermaid
flowchart TD
A[JSON解析失败] --> B[从原始文本中提取时长]
B --> C{是否找到时长信息?}
C --> |是| D[使用提取的时长值]
C --> |否| E[使用null表示无法确定]
D --> F[返回包含提取时长的基础结果]
...
提取成功后，系统会返回包含提取时长的基础结果，而不是硬编码的0值：
...
这种改进的逻辑确保了即使在JSON解析失败的情况下，系统也能尽可能地从原始文本中获取有价值的信息，提高了系统的robust性和用户体验。
...
| JSON解析失败 | 分析结果格式错误 | API返回非JSON数据 | 验证API响应格式，系统会尝试从文本中提取时长 |
...
- **增强的错误恢复**：在JSON解析失败时，能从原始文本中提取关键信息

...
<cite>
**本文档引用的文件**
- [video_analyzer.py](file://backend\src\scripts\video_analyzer.py) - *更新了视频时长提取方法和诊断功能*
- [video_diagnosis.py](file://backend\src\scripts\video_diagnosis.py) - *新增的视频诊断工具*
...
## 更新摘要
**变更内容**
- 更新了视频时长提取机制，增加了ffprobe和多种OpenCV方法
- 新增了详细的诊断信息记录功能
- 添加了视频诊断工具的相关说明
- 更新了核心组件分析和视频分析流程部分以反映最新功能
...
- [video_analyzer.py](file://backend/src/scripts/video_analyzer.py#L43-L395)
...
视频分析脚本现在采用多层方法提取视频时长，确保在各种情况下都能准确获取视频持续时间：

**视频时长提取流程**

```mermaid
flowchart TD
Start([开始时长提取]) --> CheckFile["检查文件存在性"]
CheckFile --> FileExists{文件存在?}
FileExists --> |否| RecordError["记录文件不存在错误"]
FileExists --> |是| GetFileSize["获取文件大小"]
GetFileSize --> Method1["方法1: ffprobe"]
Method1 --> FFprobeSuccess{ffprobe成功?}
FFprobeSuccess --> |是| ReturnDuration["返回ffprobe结果"]
FFprobeSuccess --> |否| Method2["方法2: OpenCV主要方法"]
Method2 --> OpenCVSuccess{OpenCV打开成功?}
OpenCVSuccess --> |否| RecordOpenCVErr["记录OpenCV错误"]
OpenCVSuccess --> |是| GetFrameInfo["获取帧率和总帧数"]
GetFrameInfo --> HasFrameInfo{有帧率和总帧数?}
HasFrameInfo --> |否| UseMethod3["使用方法3"]
HasFrameInfo --> |是| CalculateDuration["计算时长=总帧数/帧率"]
CalculateDuration --> ReturnCalculated["返回计算结果"]
UseMethod3 --> Method3["方法3: OpenCV备选方法"]
Method3 --> Method3Success{方法3成功?}
Method3Success --> |是| ReturnMethod3["返回方法3结果"]
Method3Success --> |否| AllFailed["所有方法失败"]
AllFailed --> RecordFinalError["记录最终错误信息"]
RecordFinalError --> End([返回失败状态])
```

**章节来源**
- [video_analyzer.py](file://backend\src\scripts\video_analyzer.py#L46-L125)

### 诊断信息记录

系统现在提供详细的诊断信息，帮助定位视频分析过程中的问题：
...
diagnostics = {
    ...
    "ffprobe_success": True,       # ffprobe方法是否成功
    "opencv_success": True,        # OpenCV主要方法是否成功
    "opencv_method2_success": False, # OpenCV备选方法是否成功
    "errors": []                   # 错误信息列表
}
```

当视频分析失败时，系统会记录详细的错误信息，包括：
- 文件不存在或无法访问
- 无法获取文件大小
- ffprobe调用失败
- OpenCV无法打开视频文件
...
**章节来源**
- [video_analyzer.py](file://backend\src\scripts\video_analyzer.py#L54-L61)
...
| 视频时长获取失败 | duration为0 | 视频格式不支持/文件损坏 | 使用video_diagnosis.py工具诊断 |

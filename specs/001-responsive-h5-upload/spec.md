...
用户需要通过移动端H5页面选择最多3个同类型视频文件（mp4/avi），选择**统一分类**后批量上传到对应目录。
...
2. **Given** 用户点击"开始上传"按钮, **When** 系统弹出分类选择对话框, **Then** 用户**必须选择统一分类**："个人视频"或"景区视频"
3. **Given** 用户选择"个人视频"并点击确定, **When** 系统开始上传, **Then** 所有文件上传至/backend/upload/personal/目录
4. **Given** 所有文件上传完成, **When** 系统显示"上传完成"按钮, **Then** 用户点击后跳转新页面显示"下回分解"
...
- Q: 项目复杂度和实现时间范围 → A: 保留进度条、企业级规范、良好架构、错误处理，简化为2-3天实现，30-40个合理任务
- Q: 批量上传文件分类规则 → A: 批量上传的文件只能选择一种上传分类，要么是"用户视频"，要么是"景区视频"，不可以分开选择
- Q: 文件验证规则 → A: 文件必须是mp4和avi格式，单个文件不得超过300MB，批量上传数量不得超过3个
- Q: 错误处理和安全要求 → A: 简化基础错误处理，基础安全措施，不要求企业级
- Q: 开发流程 → A: 1.先开发API接口；2.前后端根据接口同步开发；3.直接单元测试+E2E测试，使用test-videos真实文件；4.上传完成后点击按钮跳转新页面显示"下回分解"
- Q: 日志要求 → A: 必需日志系统，分用户日志和开发日志2个级别
- Q: 技术栈选择 → A: 前后端采用成熟稳定的技术栈
- Q: E2E测试方式 → A: 使用Chrome DevTools MCP工具进行可视化测试
- Q: 前端端口配置 → A: 前端固定端口号3005，配置文件中设置
- Q: 后端端口配置 → A: 后端固定端口号8005，配置文件中设置
- Q: 端口占用处理 → A: 测试前检查3005和8005端口占用，终止占用进程确保空闲
- Q: 依赖安装方式 → A: 必须使用cnpm命令安装依赖包
- Q: 错误调试方法 → A: 逐一注释代码块排除法定位错误
...
### 移动优先设计 (Principle I)
✅ **合规**: 规范明确要求H5页面支持移动端，响应式设计适配各种设备屏幕

### 渐进式视频处理 (Principle II)
✅ **合规**: 支持大文件上传(300MB)，实时进度显示，错误处理机制

### 用户体验至上 (Principle III)
✅ **合规**: 包含进度条、错误提示、加载状态、直观的操作流程

### 性能与可扩展性 (Principle IV)
✅ **合规**: 支持并发处理，日志系统监控，成熟技术栈

### 安全与隐私保护 (Principle V)
⚠️ **部分合规**: 基础文件验证和错误处理，企业级安全措施被简化
...
#### 上传路径控制
- **FR-010**: System MUST 选择"个人视频"时，文件保存至/backend/upload/personal/目录
- **FR-011**: System MUST 选择"景区视频"时，文件保存至/backend/upload/scenic/目录
...
#### 响应式设计与用户体验
- **FR-023**: System MUST 页面必须支持响应式设计，适配手机、平板、桌面等不同设备屏幕尺寸
- **FR-024**: System MUST 按钮和文字大小应根据屏幕尺寸自动调整，确保移动端可读性
- **FR-025**: System MUST 交互元素应有视觉反馈（如颜色变化、动画效果）
- **FR-026**: System MUST 异步操作应有明确的加载状态指示器
- **FR-027**: System MUST 用户操作结果应有清晰的提示信息
...
- **uploadPath**: 上传路径 (/backend/upload/personal/ 或 /backend/upload/scenic/)
...
- **SC-001**: 用户可以在60秒内完成完整操作流程：文件选择(15秒) + 分类确认(10秒) + 开始上传(35秒缓冲)
- **SC-002**: 上传进度更新延迟不超过500毫秒，用户感受到实时反馈
- **SC-003**: 批量上传完成率 > 95%
- **SC-004**: 错误提示理解率 > 90%
- **SC-005**: 用户任务完成时间 < 8分钟（包含文件选择、分类确认和上传准备）
...
- Q: 文件分类机制 → A: 提供"个人视频"(/backend/upload/personal/)和"景区视频"(/backend/upload/scenic/)两个分类
